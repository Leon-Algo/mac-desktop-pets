...
- A final ordinary launch exposed an AppKit automatic-termination race before delegate initialization. Lifecycle protection now starts before `app.run()`; three consecutive ordinary launches each reached the exact five-window healthy state.
